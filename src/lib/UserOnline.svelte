...
>
  <div class="flex items-center overflow-hidden">
    <!-- Avatar -->
    <div class="transparent-bg relative h-10 w-10">
      <img alt="{name}'s avatar" src={picture} />
    </div>
    ...
    <div class={isYou ? "pointer-events-none" : ""}>
      <sl-tooltip content="Use color">
        <button
          ...
          class="focus-visible-style transparent-bg group relative h-[40px] w-[40px] rounded-[4px]"
        >
          <span
            class="inner-border mix absolute inset-0 flex items-center justify-center rounded-[4px]"
            style="background: {brush.color};"
          >
            <span
              class="mix-blend-luminosity transition-colors {blackText
            ...
            >
              ...
              {#if tool === "fill"}
                <svg
                  class="mt-1.5 ml-0.5 h-6 w-6 scale-x-[-1]"
                  viewBox="0 0 24 24"
                >
                  <path
                    fill="currentColor"
